...
export function TarjetasPage() {
  ...
  const [pagarFecha, setPagarFecha] = useState(new Date().toISOString().split('T')[0])
        ...
        <div className="tarjeta-modal-form">
          <div>
            ...
            <select
              ...
              value={pagarBilleteraId ?? ''}
              onChange={e => setPagarBilleteraId(e.target.value ? Number(e.target.value) : null)}
              required
            >
              ...
              {filteredBilleteras.map(b => (
                <option key={b.billetera_id} value={b.billetera_id}>
                  {b.icono || '💰'} {b.nombre} ({fmtARS(b.saldo_actual)} {b.moneda})
                </option>
              ))}
            </select>
          </div>
